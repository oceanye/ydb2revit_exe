...
> 2026-09-08 修订：根据项目 1 节点 341 与 PEC 墙端 H 双源几何复核，
> 柱偏心平移向量应为原文公式的整体相反数。本修订覆盖本文此前版本；
> 定量依据见 `handoff-插件端-柱偏心Ecc整体反号修正.md`。
...
当前清单实际加载的 `CreateNewExtern.InformationEntryLifecycle.20260827.dll` 已能在柱定位路径中先把 degree 转为 radian，但平移向量采用了上述真值公式的整体相反数。Revit 端必须修正两个分量的整体符号，并把本节验算点固化为回归测试。90° 样本还应与 YJK 屏显方向进行一次对照验收。
...
| 优先级 | 修改 | 当前审计状态 |
|---|---|---|
| P0 | 修正 3.1 柱定位公式的整体符号并固化测试，覆盖偏心与旋转同时非零 | 活动 DLL 的平移向量与 YJK 真值整体反号 |
...
1. **组合偏心旋转**：输入 `(150,-175,90°)`，柱轴线平移 `(+175,-150) mm`，截面再绕新轴旋转 90°；同时与 YJK 屏显方向核对转角正向。
2. **零转角符号**：`(0,100,0°)` 的轴线沿全局 Y 正向移动 100 mm。

...
盈建科官方《杆件截面局部坐标/偏心定义》（yjk.cn/service/download/333）：
...
- 非对称截面（如 L 形柱）在 θ=0 时截面 X 轴沿 −X（镜像）——对称
  ...
  X 镜像，届时另立 handoff。
- 转角方向（逆时针为正）目前由官方文档+对称截面数据支持；若验收
  第 3 条 θ=90 样本不符（应为 +X 却见 −X），则旋转方向取反即可
  （一并在 offsetX/offsetY 的 sin 项符号上体现），同处修改。
